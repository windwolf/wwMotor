...
namespace wibot::motor
{
	...
	{
	 public:
		SixStepControlBase(PowerSensor* power_sensor,
			PhaseVoltageSensor* phase_voltage_sensor,
			...
			BusDutyReferenceUpdater* bus_duty_reference_updater,
			Modular* modular,
			Driver* driver)
			: _power_sensor(power_sensor),
			  _phase_voltage_sensor(phase_voltage_sensor),
			  _section_sensor(section_sensor),
			  _section_switcher(section_switcher),
			  _position_speed_sensor(position_speed_sensor),
			  _bus_current_reference_updater(bus_current_reference_updater),
			  _bus_duty_reference_updater(bus_duty_reference_updater),
			  _modular(modular),
			  _driver(driver)
		{
		};
	...
	class ClassicSixStepControl :
		public Configurable<ClassicSixStepControlConfig>
	{
	 public:
		ClassicSixStepControl(Driver* driver) :
			_driver(driver),
			_six_step_control(&_power_sensor,
				&_phase_voltage_sensor,
				&_zero_cross_detector,
				&_zero_cross_detector,
				&_zero_cross_detector,
				&_speed_controller,
				&_current_controller,
