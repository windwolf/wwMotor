...
class FocControlBase {
   public:
    FocControlBase(PowerSensor* power_sensor, PhaseCurrentSensor* phase_current_sensor,
                   PositionSpeedSensor* position_speed_sensor, SectionSensor* section_sensor,
                   SpeedReferenceUpdater*     speedReferenceUpdater,
                   DqCurrentReferenceUpdater* dqCurrentReferenceUpdater,
                   DqVoltageReferenceUpdater* dqVoltageReferenceUpdater, Modular* modular,
                   Driver* driver)
        : _powerSensor(power_sensor), _phaseCurrentSensor(phase_current_sensor),
          _positionSpeedSensor(position_speed_sensor), _sectionSensor(section_sensor),
          _modular(modular), _driver(driver), _dqVoltageReferenceUpdater(dqVoltageReferenceUpdater),
...
struct FocCommand {
    ...
    FocCommand(){};
    FocCommand(MotorRunMode mode) : mode(mode){};
...
class FocControl {
   public:
    ...
    void setCurrentPid(float p, float i, float d) {
        _currentController.config.useParams = false;
        _currentController.config.simple.p  = p;
        _currentController.config.simple.i  = i;
        _currentController.config.simple.d  = d;
        _currentController.apply_config();
    }
