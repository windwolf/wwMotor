...
#define CALIBRATION_SAMPLING_ROUND 50
    ...
    void AbsoluteEncoderPositionSpeedSensor::position_speed_get(Motor& motor, Vector2f& position, Vector2f& speed)
    {
        //TODO: 电角度计算是否有误.
        ...
        uint32_t step_delay) const
    {
        ...
        {
            motor.state.position.v2 =
                Math::circle_normalize(start_pos + travel / step * j);
            motor.state.position.v1 = Math::circle_normalize(motor.state.position.v2 * config.pole_pairs);
